    ...
    html.Div(id="batch-details-panel", className="hidden", children=[
        html.Div(className="grid grid-cols-1 md:grid-cols-3 gap-6", children=[
            ...
            html.Div(className="col-span-1 md:col-span-2 bg-white rounded-xl shadow-sm border border-slate-200 p-6", children=[
                ...
                html.Div(className="mt-6 pt-4 border-t", children=[
                    html.Audio(id="dash-audio-player", controls=True, className="w-full h-8")
                ])
            ])
        ])
    ])
...
app.layout = html.Div(className="p-4 md:p-8 bg-slate-50 min-h-screen text-slate-800 font-sans", children=[
    html.Div(className="max-w-7xl mx-auto", children=[
        ...
        dcc.Tabs(id="tabs", value='tab-single', className="custom-tabs font-bold mb-4", children=[
...
def run_batch_process(n_clicks):
    """Integrates the external batch processor directly into the Dash runtime."""
    if not n_clicks:
        return dash.no_update
    ...
    if not os.path.exists(excel_path):
        return "Source Excel file not found!"
...
def load_batch_table(n_clicks):
    csv_path = r"C:\Users\k02401\Downloads\batch_analysis_results.csv"
    if not os.path.exists(csv_path):
        return html.Div("No batch tracking file found. Run batch analyzer first.", className="text-red-500")
...
def update_batch_details(selected_rows, data):
    ...
    score = row.get('Total Score', 'N/A')
    sentiment = str(row.get('Sentiment Label', 'N/A')).upper()
    summary = row.get('Summary', 'No summary available.')
    risks = row.get('Risk Flags', 'None')
    url = row.get('Audio URL', '')
    
    # Risk formatting
    if pd.isna(risks) or not risks:
        risks_ui = html.Div("✅ NO RISKS", className="text-green-600 font-bold text-xs")
    else:
        risks_ui = html.Div(str(risks), className="text-red-600 border border-red-200 bg-red-50 p-2 rounded")
    
    return "block mt-6", score, sentiment, summary, risks_ui, url
